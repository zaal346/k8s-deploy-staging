      ...
      steps {
        container('git') {
          withCredentials([usernamePassword(credentialsId: 'git-credentials-acm', passwordVariable: 'GIT_PASSWORD', usernameVariable: 'GIT_USERNAME')]) {
            ...
            sh "git clone http://${GIT_USERNAME}:${GIT_PASSWORD}@localhost:3000/${env.GITHUB_ORGANIZATION}/k8s-deploy-staging"
            ...
            sh "cd k8s-deploy-staging/ && git push http://${GIT_USERNAME}:${GIT_PASSWORD}@localhost:3000/${env.GITHUB_ORGANIZATION}/k8s-deploy-staging"
          }
        }
      }
    }
